...
async function fetchMid(origin, regId) {
  ...
  // typ02 JSON은 과거 발표시각(tmFc)을 받아주지 않는다 — 이전 발표를 요청하면
  // {응답:{헤더:{결과코드:'03', 결과메시지:'데이터 없음'}}}가 돌아와 병합이 무력화된다.
  // typ01 텍스트는 과거 발표 조회가 되므로, 최신 발표만 typ02로 받고 보충은 typ01로 받는다.
  const packs = await Promise.all(TMS.map(async (tm, i) => (i === 0
    ? { land: await call('getMidLandFcst', landReg, tm), temp: await call('getMidTa', regId, tm) }
    : { land: await callT01(KEY, 'land', landReg, tm), temp: await callT01(KEY, 'temp', regId, tm) }
  )));
...
// ── 중기예보 typ01 텍스트 (과거 발표 조회용) ─────────────────────────
// typ02 JSON은 최신 발표만 조회되므로, 3일차를 메울 이전 발표는 typ01로 받는다.
// 응답은 EUC-KR이라 arrayBuffer로 받아 디코딩해야 하늘상태(wf) 한글이 깨지지 않는다.
async function callT01(KEY, kind, reg, tmFc) {
  const phpFile = kind === 'land' ? 'fct_afs_wl.php' : 'fct_afs_wc.php';
  const tmfc10 = String(tmFc || '').substring(0, 10);
  const url = `https://apihub.kma.go.kr/api/typ01/url/${phpFile}`
    + `?reg=${reg}&tmfc1=${tmfc10}&tmfc2=${tmfc10}&disp=0&help=0&authKey=${KEY}`;
  try {
    const r = await fetch(url);
    ...
    if (text.trim().startsWith('{')) {
      try { return JSON.parse(text.trim()); } catch { /* 무시 */ }
    }
    const baseDate = String(tmFc || '').substring(0, 8);
    const item = kind === 'land'
      ? parseMidLandT01(text, reg, baseDate)
      : parseMidTempT01(text, reg, baseDate);
...
function t01DayDiff(baseYmd, efYmd) {
  const p = v => Date.UTC(+v.substring(0, 4), +v.substring(4, 6) - 1, +v.substring(6, 8));
  return Math.round((p(efYmd) - p(baseYmd)) / 86400000);
}

function parseMidLandT01(text, regId, baseDate) {
  const result = { regId };
  for (const raw of String(text).split('\n')) {
    ...
    if (mod !== 'A02') continue;
    const dayOff = t01DayDiff(baseDate, tm_ef.substring(0, 8));
    if (dayOff < 3 || dayOff > 10) continue;
    const period = parseInt(tm_ef.substring(8, 10)) < 12 ? 'Am' : 'Pm';
    result[`wf${dayOff}${period}`] = wf;
    const rn = parseInt(rn_st);
    if (!isNaN(rn) && rn >= 0) result[`rnSt${dayOff}${period}`] = rn;
  }
  return result;
}

function parseMidTempT01(text, regId, baseDate) {
  const result = { regId };
  for (const raw of String(text).split('\n')) {
    const line = raw.trim();
    if (!line || line.startsWith('#') || line.startsWith('7777') || line.includes('START7777') || line.includes('END7777')) continue;
    ...
    const dayOff = t01DayDiff(baseDate, cols[2].substring(0, 8));
    if (dayOff < 3 || dayOff > 10) continue;
